...
public class TaskProfileTest {
    ...
    public static final ValidationSupportRule validationRule = new ValidationSupportRule(def.getResourceVersion(),
            ...
            List.of("dsf-task-base-1.0.0.xml",
                    "eyematics-generic-process-data-set-status-error-extension.xml",
                    "eyematics-initiate-process-structure-definition.xml",
                    "eyematics-initiate-sub-process-structure-definition.xml",
                    "eyematics-provide-process-acknowledgement-structure-definition.xml",
                    "eyematics-provide-process-structure-definition.xml",
                    "eyematics-receive-process-initiate-dic-identifier-extension.xml",
                    "eyematics-receive-process-initiate-structure-definition.xml",
                    "eyematics-receive-process-structure-definition.xml",
                    "eyematics-receive-process-close-structure-definition.xml"),
            List.of("dsf-read-access-tag-1.0.0.xml",
                    "dsf-bpmn-message-1.0.0.xml",
                    "eyematics-generic-process-data-set-status-code-system.xml",
                    "eyematics-generic-process-data-set-status-codes-code-system.xml",
                    "eyematics-receive-process-code-system.xml",
                    "eyematics-receive-process-initiate-code-system.xml"),
            List.of("dsf-read-access-tag-1.0.0.xml",
                    "dsf-bpmn-message-1.0.0.xml",
                    "eyematics-generic-process-data-set-status-codes-value-set.xml",
                    "eyematics-generic-process-data-set-status-value-set.xml",
                    "eyematics-receive-process-initiate-value-set.xml",
    ...
    @Test
    public void testValidTaskInitiateReceiveProcessOutput() {
    ...
    @Test
    public void testValidTaskInitiateReceiveProcessOutputError() {
        Task task = createValidTaskInitiateReceiveProcess();

        task.addOutput(new DataSetStatusGenerator().createDataSetStatusOutput(
                EyeMaticsGenericStatus.DATA_PROVIDE_MISSING.getStatusCode(),
                EyeMaticsConstants.CODESYSTEM_GENERIC_DATA_SET_STATUS,
                EyeMaticsConstants.CODESYSTEM_DATA_TRANSFER_VALUE_DATA_SET_STATUS));
    ...
    private Task createValidTaskInitiateProvideProcess() {
        Task task = new Task();
        task.setIdElement(new IdType("urn:uuid:" + UUID.randomUUID().toString()));
        task.getMeta().addProfile(InitiateConstants.PROFILE_TASK_INITIATE_EYEMATICS_PROVIDE_PROCESS);
        task.setInstantiatesCanonical(
                InitiateConstants.PROFILE_TASK_EYEMATICS_INITIATE_PROCESS_URI + "|" + def.getResourceVersion());
        task.setStatus(TaskStatus.REQUESTED);
        task.setIntent(TaskIntent.ORDER);
        ...
        task.getRestriction().addRecipient().setType(ResourceType.Organization.name())
                .setIdentifier(NamingSystems.OrganizationIdentifier.withValue(DIC_IDENTIFIER));
        task.addInput().setValue(new StringType(InitiateConstants.PROFILE_TASK_INITIATE_EYEMATICS_PROVIDE_PROCESS_MESSAGE_NAME))
                .getType().addCoding(CodeSystems.BpmnMessage.messageName());
        task.addInput().setValue(new StringType(UUID.randomUUID().toString())).getType()
                .addCoding(CodeSystems.BpmnMessage.businessKey());
        task.addInput().setValue(new StringType(UUID.randomUUID().toString())).getType()
    ...
    public void testValidTaskInitiateProvideProcessOutputError() {
        ...
        task.addOutput(new DataSetStatusGenerator().createDataSetStatusOutput(
                EyeMaticsGenericStatus.PROVIDE_INITIATE_FAILURE.getStatusCode(),
                EyeMaticsConstants.CODESYSTEM_GENERIC_DATA_SET_STATUS,
                EyeMaticsConstants.CODESYSTEM_DATA_TRANSFER_VALUE_DATA_SET_STATUS,
                "some error message"));
    ...
    private Task createValidTaskCloseReceiveProcessResponse() {
        Task task = new Task();
        task.getMeta().addProfile(ReceiveConstants.PROFILE_TASK_EYEMATICS_CLOSE_RECEIVE_PROCESS + "|" + def.getResourceVersion());
        task.setInstantiatesCanonical(
                ReceiveConstants.PROFILE_TASK_EYEMATICS_RECEIVE_PROCESS_URI + "|" + def.getResourceVersion());
        task.setStatus(TaskStatus.REQUESTED);
        task.setIntent(TaskIntent.ORDER);
        ...
        task.addInput().setValue(new StringType(ReceiveConstants.PROFILE_TASK_EYEMATICS_CLOSE_RECEIVE_PROCESS_MESSAGE_NAME))
                .getType().addCoding(CodeSystems.BpmnMessage.messageName());
        task.addInput().setValue(new StringType(UUID.randomUUID().toString())).getType()
                .addCoding(CodeSystems.BpmnMessage.businessKey());
        task.addInput().setValue(new StringType(UUID.randomUUID().toString())).getType()
    ...
    @Test
    public void testValidTaskCloseReceiveProcessInput() {
        Task task = createValidTaskCloseReceiveProcessResponse();
    ...
    private Task createValidTaskStartProvideProcess() {
        Task task = new Task();
    ...
    @Test
    public void testValidTaskStartProvideProcessOutput() {
        Task task = createValidTaskStartProvideProcess();
    ...
    @Test
    public void testValidTaskStartProvideProcessOutputError() {
        Task task = createValidTaskStartProvideProcess();
