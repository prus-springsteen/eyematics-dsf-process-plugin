...
public class InitiateConfig {
    ...
    @Bean
    @Scope(ConfigurableBeanFactory.SCOPE_PROTOTYPE)
    public RequestDataMessageTask requestDataMessageTask() { return new RequestDataMessageTask(api); }

    @Bean
    @Scope(ConfigurableBeanFactory.SCOPE_PROTOTYPE)
    public FinalizeInitiateProcessTask finalizeInitiateProcessTask() { return new FinalizeInitiateProcessTask(api); }
}
